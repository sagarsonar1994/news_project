...
class PostController extends Controller {
	...
	public function get() {
		...
		return datatables()->of($posts)
			...
			->editColumn('post_title', '{{ $post_title }}')
...
// 			'post_slug' => 'required|alpha_dash|min:5|max:150|unique:posts',
            'post_slug' => 'required|min:5|max:150|unique:posts',
		...
		$newMessageBody = '';
		$message = request()->get('post_details');
		if($message) {
			$dom = new \DomDocument();
			...
			$dom->encoding = 'UTF-8'; // insert proper (sets all three)
			...
			foreach ($images as $image) {
	            ...
	            if (preg_match('/data:image/', $imageSrc)) {
	                 ...
	                 * public/uploads
	                 */
	                $filePath = "/featured_image/$filename.$mimeType";
	                ...
	                $newImageSrc = asset('public'.$filePath);
        ...
        $post_slug = str_replace(' ', '-', $request->input('post_slug'));
		$post = Post::create([
			...
			'post_details' => Purifier::clean($newMessageBody),
	...
	public function show($id) {
		$post = Post::with(['category:id,category_name', 'user:id,name', 'tags:id,tag_name'])->where('id', $id)
			->first();
	...
	public function update(Request $request, $id) {
		...
		$post->post_details = Purifier::clean($newMessageBody);
