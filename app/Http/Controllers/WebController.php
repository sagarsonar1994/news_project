...
class WebController extends Controller {
	...
	public function index() {
		...
		$lastest_posts = Post::where(['publication_status' => 1])->orderBy('id', 'desc')->limit(5)->get();
		

		$trandingTag = Tag::where(['publication_status' => 1, 'tag_name' => 'ट्रेंडिंग न्यूज'])->first();
		$trandingPosts = $trandingTag->posts()->where('publication_status', 1)->orderBy('post_date', 'desc')->limit(8)->get();

		//बड़ी खबरें start here
		$BigNewsTag = Tag::where(['publication_status' => 1, 'tag_name' => 'बड़ी खबरें'])->first();
		$BigNewsPosts = $BigNewsTag->posts()->where('publication_status', 1)->orderBy('post_date', 'desc')->limit(4)->get();


		//Desh news start here
		$deshCategory = Category::where(['publication_status' => 1, 'category_slug' => 'india'])->first();
		$DeshPosts = $deshCategory->post()->where(['publication_status' => 1])->orderBy('post_date', 'desc')->limit(4)->get();


		//DUNIYA news start here
		$duniyaCategory = Category::where(['publication_status' => 1, 'category_slug' => 'world'])->first();
		$duniyaPosts = $duniyaCategory->post()->where(['publication_status' => 1])->orderBy('post_date', 'desc')->limit(4)->get();



		//state news start here
		$stateCategory = Category::where(['publication_status' => 1, 'category_slug' => 'state'])->first();
		$statePosts = $stateCategory->post()->where(['publication_status' => 1])->orderBy('post_date', 'desc')->limit(4)->get();



		//khel news start here
		$sportCategory = Category::where(['publication_status' => 1, 'category_slug' => 'sports'])->first();
		$sportPosts = $sportCategory->post()->where(['publication_status' => 1])->orderBy('post_date', 'desc')->limit(7)->get();


		$entertainmentCat = Category::where(['publication_status' => 1, 'category_slug' => 'entertainment'])->first();

		$entertainmentPosts = $entertainmentCat->post()->where(['publication_status' => 1])->orderBy('post_date', 'desc')->limit(7)->get();


		$photoCat = Category::where(['publication_status' => 1, 'category_slug' => 'photos'])->first();
		$photoPosts = $photoCat->post()->where(['publication_status' => 1])->orderBy('post_date', 'desc')->limit(7)->get();


		$businessCat = Category::where(['publication_status' => 1, 'category_slug' => 'business'])->first();
		$businessPosts = $businessCat->post()->where(['publication_status' => 1])->orderBy('post_date', 'desc')->limit(4)->get();
		...
		// dd($entertainmentPosts);

		$category_array = [];
		$categories = Category::where(['publication_status' => 1])->where('category_slug','!=','state')->orderBy('id','desc')->get(['id', 'category_name']);
		
// 		dd($categories);
		foreach($categories as $key => $category){
			$category = Category::where(['publication_status' => 1, 'id' => $category->id])->first();
			$posts = $category->post()->where(['publication_status' => 1])->orderBy('post_date', 'desc')->limit(4)->get();

			$category_array[$key]['category_name'] = $category->category_name;
			$category_array[$key]['posts'] = $posts;
		}
		
		$categories = Category::where(['publication_status' => 1])->where('category_slug','=','state')->orderBy('id','desc')->get(['id', 'category_name']);
		foreach($categories as $key => $category){
			$category = Category::where(['publication_status' => 1, 'id' => $category->id])->first();
			$posts = $category->post()->where(['publication_status' => 1])->orderBy('post_date', 'desc')->limit(7)->get();
		...
		$b_tag = Tag::where(['publication_status' => 1, 'tag_name' => 'ब्रेकिंग न्यूज'])->first();
		$breaking_posts = $b_tag->posts()->where('publication_status', 1)->orderBy('post_date', 'desc')->limit(10)->get();


		return view('front.index', compact('posts', 'popular_posts', 'lastest_posts', 'setting', 'category_array','breaking_posts','trandingPosts', 'BigNewsPosts','DeshPosts','duniyaPosts','statePosts', 'sportPosts','entertainmentPosts','photoPosts','businessPosts'));
	}
	...
	public function tags() {
		...
		return view('web.tags', compact('tags','breaking_posts'));
	}
	...
	public function categoryDetail($slug) {
		$page = Page::where(['publication_status' => 1, 'category_name' => $slug])->first();
		if($page) {
		    $id = $page->id;
		...
		$tag = Tag::where(['publication_status' => 1, 'tag_name' => 'बड़ी खबरें'])->first();
		$posts = $tag->posts()->where('publication_status', 1)->orderBy('post_date', 'desc')->limit(30)->get();
		// return view('web.category', compact('posts', 'popular_posts', 'category'));
		// dd($posts);
		$b_tag = Tag::where(['publication_status' => 1, 'tag_name' => 'ब्रेकिंग न्यूज'])->first();
		$breaking_posts = $b_tag->posts()->where('publication_status', 1)->orderBy('post_date', 'desc')->limit(10)->get();
		
        return view('front.category_news', compact('posts', 'popular_posts', 'category','breaking_posts'));
		}
	}
	...
	public function category($id) {
		...
		$tag = Tag::where(['publication_status' => 1, 'tag_name' => 'बड़ी खबरें'])->first();
		$posts_b = $tag->posts()->where('publication_status', 1)->orderBy('post_date', 'desc')->limit(15)->get();
		// return view('web.category', compact('posts', 'popular_posts', 'category'));
		// dd($posts);
		$b_tag = Tag::where(['publication_status' => 1, 'tag_name' => 'ब्रेकिंग न्यूज'])->first();
		$breaking_posts = $b_tag->posts()->where('publication_status', 1)->orderBy('post_date', 'desc')->limit(10)->get();
        ...
        return view('front.category_news', compact('posts', 'lastest_posts', 'category','breaking_posts','posts_b'));
	}
	...
	public function tag($id) {
		...
		return view('front.tag_news', compact('tag', 'lastest_posts', 'posts','breaking_posts','posts_b'));
	}
	...
	public function details($slug) {
		...
		if (!empty($post)) {
			...
			$tag = Tag::where(['publication_status' => 1, 'tag_name' => 'बड़ी खबरें'])->first();
			$posts = $tag->posts()->where('publication_status', 1)->orderBy('post_date', 'desc')->limit(15)->get();

		
			$popular_posts = Post::where(['publication_status' => 1])->orderBy('view_count', 'desc')->limit(5)->get();

			$categories = Category::where('publication_status', 1)->get(['id', 'category_name']);
			$tags = Tag::where('publication_status', 1)->get(['id', 'tag_name']);
			
			return view('front.details', compact('post', 'related_posts', 'lastest_posts', 'posts', 'comments','categories', 'tags'));
		} else {
	...
	public function advertisePage(Request $request) {
